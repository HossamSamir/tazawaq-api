...
function travers(req, res) {
	sql.qry(
		'SELECT id,store_id, name, info, status, img FROM offers',
		offers => {
...
app.post('/add-offer', function(req, res) {
	...
	if (
		...
		sql.qry(
			'INSERT INTO offers (name, info, status, store_id, img) VALUES(?,?,?,?,"")',
			[name, info, status, store_id, image],
			function(response) {
...
app.post('/edit-offer', function(req, res) {
	...
	if (!name || !info || !status || !store_id) {
		res.send('هناك مدخلات ناقصة او لم تُكتب بشكل صحيح من فضلك راجعها');
	} else {
		sql.qry(
			'UPDATE offers SET name = ?, info = ?, status = ?, store_id = ? WHERE id = ?',
			[name, info, status, store_id, offer_id],
			function(response) {
				if (image != null) {
