	...
	sql.qry('SELECT id,cost,info,location,store_id,status,user_id,note,cost_dicounted FROM orders WHERE store_id=? and status <= 1 ORDER BY status ASC, id DESC', [store_id], function(orders_res) {
		...
		async.forEachOf(orders_res, function (order, i, callback) {
			sql.qry('SELECT phone,username FROM users WHERE id=? LIMIT 1', [ order.user_id ], function(userData) {
				...
				if(userData.length)
					orders.push( [ userData[0].phone, order.location, order.info, order.status,order.id,order.note,userData[0].username,order.cost_dicounted,store[0].delivery_cost,userData[0].phone] );
				else
					orders.push( [ 'غير متاح', order.location, order.info, order.status,order.id,order.note,'غير متاح',order.cost_dicounted,store[0].delivery_cost,userData[0].phone ] );
				callback(null);
				})
			});
		}, function(err) {
