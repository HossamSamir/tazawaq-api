function travers(req, res) {
	...
	sql.qry(
		'SELECT id,cost,delivery_cost,info,location,user_id,status,TIMESTAMPDIFF(MINUTE,time_ordered,NOW()) AS elapsed_time,note,cost_dicounted FROM orders WHERE store_id=? and status<2 ORDER BY id DESC',
		[store_id],
		function(orders_res) {
			var orders = [];
			async.forEachOf(
