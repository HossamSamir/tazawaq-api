function travers(req, res) {
	...
	sql.qry(
		'SELECT id,cost,info,location,user_id,status,store_id,TIMESTAMPDIFF(MINUTE,time_ordered,NOW()) AS elapsed_time FROM orders WHERE store_id=? ORDER BY id DESC',
		[store_id],
		function(orders_res) {
			var orders = [];
			async.forEachOf(
